...
@Component({
  ...
  styleUrls: ['./gauge3.component.css']
})
export class Gauge3Component implements OnInit {
  gaugemap:any = {};
  constructor() { }
    ngOnInit() {
    this.draw();
...
hello;
  draw() {
    ...
    var gauge = function (container, configuration) {
      ...
      .enter().append('text')
				...
				.attr('transform', function(d) {
					...
					var ratio = scale(d);
					var newAngle = config.minAngle + (ratio * range);
					return 'rotate(' +newAngle +') translate(-1.5,' +(config.labelInset - 32) +')';
        })
        .attr('font-size','7px')
        .attr('fill','grey')
        .text('|');
    ...
    var powerGauge:any = gauge('#power-gauge3', {
      size: 100,
      clipWidth: 80,
      clipHeight: 45,
      ringWidth: 60,
